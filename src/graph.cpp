...
Graph::Graph(xt::xarray<float> &img) {
  ...
  cv::Mat img_bgr =
      cv::Mat(img.shape()[0], img.shape()[1], CV_32FC3, img.data());
  cv::Mat img_yuv;
  ...
  for (std::size_t i = 0; i < height; ++i) {
    for (std::size_t j = 0; j < width; ++j) {
      ...
      for (int k = 0; k < 8; ++k) {
        ...
        int ni = i + ((k == 1 || k == 2 || k == 3)   ? -1
                      : (k == 5 || k == 6 || k == 7) ? 1
                                                     : 0);
        int nj = j + ((k == 0 || k == 1 || k == 7)   ? 1
                      : (k == 3 || k == 4 || k == 5) ? -1
                                                     : 0);
...
xt::xarray<bool> Graph::get_neighbours() const { return m_neighbours; }

xt::xarray<float> Graph::get_image() { return m_img; }

void Graph::remove_trivial_edges() {
  // this function removes the diagonal edges when all 4 corners of a square are
  // the same colors (flat shaded region)
  std::size_t height = m_img.shape()[0];
  std::size_t width = m_img.shape()[1];
  // Iterate over each pixel
  for (std::size_t i = 0; i < height - 1; ++i) {
    for (std::size_t j = 0; j < width - 1; ++j) {
      ...
      if (m_neighbours(i, j, 7) && m_neighbours(i + 1, j, 1) &&
          is_close_color(color_1, color_2)) {
...
bool Graph::is_close_color(const xt::xarray<float> &color_pixel,
                           const xt::xarray<float> &color_neighboor) {
  ...
  return (delta_Y <= Y_THRESHOLD && delta_U <= U_THRESHOLD &&
          delta_V <= V_THRESHOLD);
}
...
cv::Mat Graph::draw_neighbours() {
  ...
  // Upscale the image
  cv::Mat output_image;
  cv::resize(img_bgr, output_image, cv::Size(), scale_factor, scale_factor,
             cv::INTER_NEAREST);

  // Draw grid lines (gray color)
  for (int y = 0; y <= output_image.rows; y += scale_factor) {
    cv::line(output_image, cv::Point(0, y), cv::Point(output_image.cols, y),
             cv::Scalar(192, 192, 192), 1);
  }
  for (int x = 0; x <= output_image.cols; x += scale_factor) {
    cv::line(output_image, cv::Point(x, 0), cv::Point(x, output_image.rows),
             cv::Scalar(192, 192, 192), 1);
  }
  ...
  for (int y = 0; y < rows; ++y) {
    for (int x = 0; x < cols; ++x) {
      ...
      std::vector<bool> n = {m_neighbours(y, x, 0), m_neighbours(y, x, 1),
                             m_neighbours(y, x, 2), m_neighbours(y, x, 3),
                             m_neighbours(y, x, 4), m_neighbours(y, x, 5),
                             m_neighbours(y, x, 6)};
      ...
      for (size_t i = 0; i < n.size(); ++i) {
        if (n[i]) {
          ...
          // Current pixel center (scaled and shifted to the center of the
          // pixel)
          cv::Point current_pixel(x * scale_factor + scale_factor / 2,
          ...
          for (size_t i = 0; i < n.size(); ++i) {
            if (n[i]) {
              ...
              cv::Point neighbor_pixel(neighbor_coords[i].x + scale_factor / 2,
                                       neighbor_coords[i].y + scale_factor / 2);

              // Check bounds before drawing
              if (neighbor_pixel.x >= 0 &&
                  neighbor_pixel.x < output_image.cols &&
                  neighbor_pixel.y >= 0 &&
                  neighbor_pixel.y < output_image.rows) {
                cv::line(output_image, current_pixel, neighbor_pixel,
                         cv::Scalar(0, 0, 255), 1);
              }
            }
          }
    ...
    // std::string file_path = "visualisation/output_image_" + std::to_string(y)
    // + ".png"; cv::imwrite(file_path, output_image);
  }

  // Optionally downscale the final image
  // cv::resize(output_image, output_image, img_bgr.size(), 0, 0,
  // cv::INTER_NEAREST);
